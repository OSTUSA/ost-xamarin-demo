...
namespace OSTUSA.XamarinDemo.Services.Networking.Mqtt.Messages
{
    /// <summary>
    ...
    public class MqttMsgPublish : MqttMsgBase
    {
        ...
        public static MqttMsgPublish Parse(byte fixedHeaderFirstByte, byte protocolVersion, IMqttNetworkChannel channel, ref bool isRunning)
        {
            byte[] buffer;
            int index = 0;
            byte[] topicUtf8;
            ...
            int received = channel.Receive(buffer, ref isRunning);
            if (!isRunning)
                return msg;
            ...
            while (remaining > 0)
            {
                ...
                received = channel.Receive(buffer, ref isRunning);
                if (!isRunning)
                    return msg;
